...
* [`src/kanban_sim/`](src/kanban_sim/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/kanban_sim/core.cljs): contains the SPA entry point, `init`
* [`test/kanban_sim/`](test/kanban_sim/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `kanban-sim` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/kanban_sim/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns kanban-sim.example
  (:require [kanban-sim.config :as config])
